...
async function generateMemoInBackground(
  ...
  try {
    ...
    // New memos only: apply the new provenance + two-pass tone model
    // ONLY when there is no prior memo for this company.
    // =============================================================
    const MEMO_METHODOLOGY_VERSION = "v2_two_pass_provenance";
    const useMethodologyV2 = !existingMemo;
    ...
    // ============================================
    // Provenance separation helpers (no schema changes)
    // ============================================
    const isManualEvidenceResponse = (r: any) => {
      const src = (r?.source ?? "manual").toLowerCase();
      return src === "manual";
    };

    const splitResponses = (rows: any[]) => {
      const manual = (rows || []).filter(isManualEvidenceResponse);
      const nonManual = (rows || []).filter((r) => !isManualEvidenceResponse(r));
      return { manual, nonManual };
    };

    const buildNonManualContext = (rows: any[], sectionName: string) => {
      if (!rows?.length) return "";
      // Keep it tight and clearly labeled so the model cannot treat it as founder-asserted.
      const lines = rows
        .filter((r) => {
          const key = String(r?.question_key ?? "");
          // Prefer section-local keys (prefix match), plus merged keys mapping already handled above.
          // This is a heuristic to keep context relevant.
        ...
        .map((r) => {
          ...
          return `- ${r.question_key}: ${String(r.answer ?? "").slice(0, 800)} [source=${src}${conf}]`;
        })
        .join("\n");

      if (!lines.trim()) return "";
      return `\n\n--- NON-FOUNDER CONTEXT (NOT hard evidence) ---\nThe following content was imported or generated by the system (e.g., deck import, tools, calculators).\nRules:\n- Do NOT attribute these statements to the founder unless the same claim is present in MANUAL responses.\n- Do NOT penalize the company for any numbers that appear only here; treat as hypothesis/input gap.\n- You may use this ONLY to propose diligence questions or scenario framing (not as negative evidence).\n\n${lines}\n--- END NON-FOUNDER CONTEXT ---`;
    };

    const twoPassGuidelines = useMethodologyV2
      ? `\n\n=== METHODOLOGY v2 (TWO-PASS + PROVENANCE DISCIPLINE) ===\nYou are doing TWO distinct jobs and you must keep them separate:\nPASS 1 (Neutral baseline): only summarize what is known from MANUAL founder-provided evidence. Clearly separate Known vs Unknown vs Derived. Do not inject additional skepticism or verdict language.\nPASS 2 (Risk & diligence notes): list the 3-5 most decision-relevant risks or unknowns, each tied to (a) missing-but-required-for-stage vs missing-but-not-yet-expected, and (b) a specific diligence question and what evidence would resolve it.\n\nPROVENANCE RULES:\n- MANUAL responses are the only "hard evidence".\n- Any imported/system content must be treated as context/hypothesis; never as a founder claim.\n- Never criticize the company for a metric value that is model-derived or AI-estimated; instead, critique the uncertainty caused by missing data.\n\nTONE RULES:\n- Section pages must NOT take a final investment stance. Reserve any final recommendation/verdict for the Investment Thesis only.\n=== END METHODOLOGY v2 ===\n`
      : "";
    ...
    for (const sectionName of sectionOrder) {
      ...
      // Combine responses in the section.
      // For methodology v2 we separate MANUAL evidence from non-manual context.
      const { manual: manualResponses, nonManual: nonManualResponses } = splitResponses(responses || []);
      const manualSectionEntries = Object.entries(sectionResponses)
        .filter(([questionKey]) => manualResponses.some((r) => r.question_key === questionKey))
        .map(([_, answer]) => answer)
        .filter(Boolean);
      const combinedContent = (useMethodologyV2 ? manualSectionEntries : Object.values(sectionResponses))
        .filter(Boolean)
        .join("\n\n");
      ...
      // Provide non-manual context only as clearly-labeled, non-evidentiary input
...
${marketContext ? 'IMPORTANT: Leverage the AI-deduced market intelligence above to enrich your analysis.\n\n' : ''}${sectionName === "Competition" && competitorResearch ? 'CRITICAL: You MUST use the AI-researched competitor names and data above. Be honest about competitive positioning — if the company faces strong, well-funded competitors, acknowledge the challenges.\n\n' : ''}${companyModel ? 'CRITICAL: Use the Company Model context above to inform your relational reasoning. Address any coherence issues that affect this section.\n\n' : ''}Return ONLY valid JSON with this exact structure:
{
  ...
  "vcReflection": {
    ...
    "conclusion": "Lead with primary concern. Rate confidence (Low/Medium/High)."
      ...
      console.log(`Generating section: ${sectionName} (${Object.keys(sectionResponses).length} questions) methodology=${useMethodologyV2 ? MEMO_METHODOLOGY_VERSION : 'v1'}`);
        ...
        response = await fetchWithRetry("https://ai.gateway.lovable.dev/v1/chat/completions", {
          ...
          body: JSON.stringify({
            ...
            messages: [
              {
                role: "system",
                content: useMethodologyV2
                  ? `You are a VC analyst writing an internal memo.

METHODOLOGY v2 (TWO-PASS):
- PASS 1 must be a neutral baseline based ONLY on MANUAL founder-provided evidence.
- PASS 2 must be risk & diligence notes: specific unknowns, stage-appropriate expectations, and what evidence would change the view.
- Do NOT take a final investment stance in non-thesis sections.

PROVENANCE:
- Treat any imported/system context as hypothesis only; do not attribute it to founders.
- Never penalize the company for model-derived or AI-estimated numbers; critique uncertainty caused by missing evidence.

Return valid JSON only (no markdown).`
                  : `You are a PARTNER at a top-tier VC firm writing an internal investment memo to your fellow partners. 
...
{"narrative":{"paragraphs":[{"text":"Main analysis paragraph here.","emphasis":"high"},{"text":"Supporting details.","emphasis":"normal"}],"keyPoints":["Key point 1","Key point 2","Key point 3"]},"vcReflection":{"analysis":"Critical assessment.","questions":[{"question":"Key question 1?","vcRationale":"Why VCs care about this","whatToPrepare":"Evidence to address this"},{"question":"Key question 2?","vcRationale":"Why VCs care about this","whatToPrepare":"Evidence to address this"}],"conclusion":"Summary conclusion."}}`,
    ...
    const { manual: manualResponsesForThesis, nonManual: nonManualResponsesForThesis } = splitResponses(responses || []);
    const allResponsesText = manualResponsesForThesis
      .map(r => `${r.question_key}: ${r.answer || "N/A"}`)
      .join("\n\n") || "";
    const nonManualResponsesText = useMethodologyV2
      ? nonManualResponsesForThesis
          .slice(0, 40)
          .map(r => `${r.question_key}: ${r.answer || "N/A"} [source=${r.source || 'unknown'}${typeof r.confidence_score === 'number' ? ` conf=${r.confidence_score}` : ''}]`)
          .join("\n\n")
      : "";
...
${useMethodologyV2 && nonManualResponsesText ? `\n\n--- NON-FOUNDER CONTEXT (NOT hard evidence) ---\n${nonManualResponsesText}\n--- END NON-FOUNDER CONTEXT ---` : ''}
...
---
 ...
 ${useMethodologyV2 ? 'METHODOLOGY v2: You MAY take a final investment stance here, but you must explicitly separate (1) evidence-backed facts (manual), (2) hypotheses/assumptions, and (3) opinion. If evidence is insufficient, your stance must reflect that uncertainty.' : ''}
