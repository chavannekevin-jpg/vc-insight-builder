...
async function generateMemoInBackground(
  ...
  try {
    ...
    // BUGFIX: Expanded to include deck_import as valid founder evidence
    // All sources (manual, deck_import, etc.) are treated as unified "company evidence"
    // for user-facing content - no source labels shown in final output
    // ============================================
    ...
    const splitResponses = (rows: any[]) => {
      const founderEvidence = (rows || []).filter(isFounderEvidenceResponse);
      const nonFounderEvidence = (rows || []).filter((r) => !isFounderEvidenceResponse(r));
      return { founderEvidence, nonFounderEvidence };
    };

    const buildNonFounderContext = (rows: any[], sectionName: string) => {
      if (!rows?.length) return "";
      // Only system-generated content that was NOT confirmed by founder goes here
      const lines = rows
        .filter((r) => {
        ...
        .slice(0, 12)
        .map((r) => {
          // No source labels in output - all content is "enhanced by AI and Knowledge Library"
          return `- ${r.question_key}: ${String(r.answer ?? "").slice(0, 800)}`;
        })
        .join("\n");
...
--- END ADDITIONAL FOUNDER CONTEXT ---`
    ...
    for (const sectionName of sectionOrder) {
      ...
      // For methodology v2 we include all founder evidence (manual + deck_import + etc.)
      const { founderEvidence, nonFounderEvidence } = splitResponses(responses || []);
      const founderSectionEntries = Object.entries(sectionResponses)
        .filter(([questionKey]) => founderEvidence.some((r: any) => r.question_key === questionKey))
        .map(([_, answer]) => answer)
        .filter(Boolean);
      const combinedContent = (useMethodologyV2 ? founderSectionEntries : Object.values(sectionResponses))
        .filter(Boolean)
        .join("\n\n");
      ...
      // Provide non-founder context only as supplementary input
      const nonFounderContextStr = useMethodologyV2
        ? buildNonFounderContext(nonFounderEvidence, sectionName)
        : "";
      ...
      const founderQuestionKeys = new Set(founderEvidence.map((r: any) => r.question_key as string));
      const evidencePacketStr = useMethodologyV2
        ? buildEvidencePacket({
            sectionName,
            sectionResponses,
            manualQuestionKeys: founderQuestionKeys,
            nonManualRows: nonFounderEvidence,
...
{"narrative":{"paragraphs":[{"text":"Main analysis paragraph here.","emphasis":"high"},{"text":"Supporting details.","emphasis":"normal"}],"keyPoints":["Key point 1","Key point 2","Key point 3"]},"vcReflection":{"analysis":"Critical assessment.","questions":[{"question":"Key question 1?","vcRationale":"Why VCs care about this","whatToPrepare":"Evidence to address this"},{"question":"Key question 2?","vcRationale":"Why VCs care about this","whatToPrepare":"Evidence to address this"}],"conclusion":"Summary conclusion."}}`,
    ...
    const { founderEvidence: founderEvidenceForThesis, nonFounderEvidence: nonFounderEvidenceForThesis } = splitResponses(responses || []);
    const allResponsesText = founderEvidenceForThesis
      .map((r: any) => `${r.question_key}: ${r.answer || "N/A"}`)
      .join("\n\n") || "";
    const nonFounderResponsesText = useMethodologyV2
      ? nonFounderEvidenceForThesis
          .slice(0, 40)
          .map((r: any) => `${r.question_key}: ${r.answer || "N/A"}`)
          .join("\n\n")
      : "";
...
${useMethodologyV2 && nonFounderResponsesText ? `\n\n--- SUPPLEMENTARY CONTEXT ---\n${nonFounderResponsesText}\n--- END SUPPLEMENTARY CONTEXT ---` : ''}
