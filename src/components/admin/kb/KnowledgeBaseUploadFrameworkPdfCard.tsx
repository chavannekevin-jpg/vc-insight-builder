...
export function KnowledgeBaseUploadFrameworkPdfCard({
  onImported,
}: {
  onImported?: () => void;
}) {
  ...
  const handleUploadPdfAndParse = async () => {
    ...
    try {
      // 1) Create draft source (framework)
      ...
      const objectPath = `kb/frameworks/${sourceId}/${Date.now()}-${safeName}`;
      ...
      toast({
        title: "Imported",
        description: "Framework extracted and published (Active).",
      });
  ...
  return (
    <ModernCard>
      <div className="space-y-2">
        <h2 className="text-lg font-semibold text-foreground">Upload framework PDF (Europe)</h2>
        <p className="text-sm text-muted-foreground">
          Upload a PDF framework/playbook; we’ll extract a freeform methodology summary and mark it{" "}
          <span className="font-medium text-foreground">Active</span> so it can guide memo analysis.
        </p>
      </div>
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
        <div className="space-y-2">
          ...
          <Input
            ...
            placeholder="e.g., SaaS GTM Framework"
          />
        </div>
        ...
        <div className="space-y-2">
          ...
          <Input
            ...
            placeholder="e.g., a16z / Sequoia / Internal"
          />
        </div>
      </div>
