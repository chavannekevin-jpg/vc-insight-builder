...
import { Gauge, TrendingUp, DollarSign, Users, ArrowLeftRight } from "lucide-react";
    ...
    >
      ...
      <div className="flex items-center gap-4 mb-6">
        <div className="relative w-24 h-24">
          <svg className="w-24 h-24 transform -rotate-90">
            ...
            <circle 
              ...
              strokeWidth="8" 
              fill="none" 
              strokeDasharray={`${currentData.overallScore * 2.64} 264`}
              className={getScoreColor(currentData.overallScore)}
            />
          </svg>
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            <span className={cn("text-2xl font-bold", getScoreColor(currentData.overallScore))}>
        ...
        <div>
          ...
          <p className={cn("text-sm font-medium mt-1", getScoreColor(currentData.overallScore))}>
            {currentData.overallScore >= 80 ? "Excellent timing" : 
             currentData.overallScore >= 60 ? "Good timing" :
             currentData.overallScore >= 40 ? "Uncertain timing" : "Early/risky timing"}
          </p>
        </div>
      </div>
        ...
        {factors.map((factor) => (
          <div key={factor.key} className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <span className={cn("text-sm font-semibold", getScoreColor(factor.data.score))}>
                {factor.data.score}/100
              </span>
            </div>
            <div className="relative h-2 bg-muted rounded-full overflow-hidden">
              <div 
                className={cn("absolute left-0 top-0 h-full rounded-full transition-all duration-500", getScoreBg(factor.data.score))}
                style={{ width: `${factor.data.score}%` }}
              />
            </div>
            <p className="text-xs text-muted-foreground">{factor.data.evidence}</p>
          </div>
        ))}
      </div>
    </EditableToolCard>
