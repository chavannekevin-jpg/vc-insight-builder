...
interface MemoShareDialogProps {
  open: boolean;
...
export function MemoShareDialog({
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .insert({
          ...
          share_type: "simplified",
        });

      if (error) throw error;

      toast.success("Share link created! You can now share your memo with your network.");
      fetchShareLinks();
    } catch (error) {
  ...
  const toggleLinkActive = async (linkId: string, isActive: boolean) => {
    try {
      ...
      toast.success(isActive ? "Link deactivated" : "Link reactivated");
    } catch (error) {
  ...
  const copyToClipboard = async (token: string) => {
    const url = `${window.location.origin}/shared/simplified-memo/${token}`;
  ...
  const getShareUrl = (token: string) => `${window.location.origin}/shared/simplified-memo/${token}`;
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Create shareable links for <strong>{companyName}</strong>'s VC Memorandum.
            Share with your network, advisors, or potential investors.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6 mt-4">
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <h4 className="font-medium text-sm">Your Share Links</h4>
              <Button 
                variant="ghost" 
                size="sm" 
            ...
            {loading ? (
              ...
              <div className="text-center py-8 text-muted-foreground text-sm">
                No share links yet. Create one above to share your memo.
              </div>
            ) : (
